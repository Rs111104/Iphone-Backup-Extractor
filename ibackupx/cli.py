...
from ibackupx.utils import humanize_bytes, wipe_passphrase

logger = logging.getLogger(__name__)


def _summary_table(title: str, data: dict[str, object]) -> Table:
...
def main(
    ...
    if env_pass:
        passphrase = bytearray(env_pass, "utf-8")
        env_pass = ""
    ...
    actions_selected = any([do_extract, do_duplicates, do_repair, do_all])
        ...
        if do_duplicates:
            ...
            if not groups:
                console.print(
                    _summary_table("Duplicate summary", {"Groups found": 0, "Files removed": 0, "Space freed": "0 B"})
                )
            else:
                ...
                console.print(
                    f"Potential removal: {remove_count} files, {humanize_bytes(space_freed)}"
                )
                ...
                console.print(
                    _summary_table(
                        "Duplicate summary",
                        {
                            ...
                            "Space freed": humanize_bytes(summary.space_freed_bytes),
                        },
                    )
                )
